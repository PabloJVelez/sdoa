...
export const SelectedProducts = ({ selectedProducts }: SelectedProductsProps) => {
  ...
  const { data: products, isLoading, error } = useQuery<ProductInfo[]>({
    ...
    queryFn: async () => {
      // Fetch products using Medusa JS SDK client
      // Access the client from the admin SDK
      const results = await Promise.all(
        productIds.map(async (id) => {
          try {
            // Use the admin client to fetch product
            const response = await (sdk.admin as any).client.fetch<{ product: ProductInfo }>(`/admin/products/${id}`, {
              method: 'GET',
            })
            ...
            return { id, title: `Product ${id.substring(0, 12)}...`, handle: undefined, thumbnail: undefined }
          }
        })
      )
      return results
  ...
  if (isLoading) {
    return (
      <div className="space-y-3">
        <div className="flex items-center justify-between mb-4">
          <Label className="text-base font-semibold">Selected Products</Label>
          <Badge>{selectedProducts?.length || 0} item{(selectedProducts?.length || 0) !== 1 ? 's' : ''}</Badge>
        </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-4">
        <Label className="text-base font-semibold">Selected Products</Label>
        <Badge>{selectedProducts.length} item{selectedProducts.length !== 1 ? 's' : ''}</Badge>
      </div>
      
      <div className="space-y-2">
        {selectedProducts.map((selected, index) => {
          const product = products?.find((p) => p.id === selected.product_id)
          // Use product title if available, otherwise show a loading/fallback state
          const productTitle = product?.title 
            ? product.title 
            : isLoading 
              ? "Loading..." 
              : `Product ${selected.product_id.substring(0, 12)}...`
          ...
          return (
            ...
            >
              ...
              <div className="flex-shrink-0 ml-4">
                <Badge variant="secondary">Qty: {selected.quantity}</Badge>
              </div>
            </div>
          )
        })}
