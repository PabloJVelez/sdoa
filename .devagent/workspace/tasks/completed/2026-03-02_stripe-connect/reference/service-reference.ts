/**
 * REFERENCE: Stripe Connect Payment Provider Service
 *
 * This file is a copy of the service implementation from the sibling project
 * (ChefV / medusa2-chefV) for use as implementation reference only.
 * Do not import or run this file in the SDOA app; use it to implement
...
class StripeConnectProviderService extends AbstractPaymentProvider<StripeConnectProviderOptions> {
  ...
  private calculateApplicationFee(amount: number): number {
    ...
    // Gross up: add estimated Stripe fees so platform nets the full feePercent
    ...
    try {
      ...
      this.logger_.info(
        `${StripeConnectProviderService.LOG_PREFIX} Created PaymentIntent ${paymentIntent.id}: ${amountInCents} ${currency_code}` +
          (this.isConnectEnabled() ? `, fee: ${applicationFeeAmount} (${this.config_.feePercent}%)` : ''),
      );
      ...
      const paymentData: StripeConnectPaymentData = {
        ...
        amount: amountInCents,
        currency: currency_code.toLowerCase(),
        connected_account_id: this.isConnectEnabled() ? this.config_.connectedAccountId : undefined,
   ...
   *
   * Session correlation: Uses `resource_id` from PaymentIntent metadata as the
   * primary session identifier (this is Medusa's payment session ID set during
   * initiatePayment). Falls back to `session_id` metadata, then PaymentIntent ID.
   */
